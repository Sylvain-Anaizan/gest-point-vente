...
export default function VentesEdit({
    ...
    const updateQuantite = (index: number, nouvelleQuantite: number) => {
        ...
        const stockDisponible = ligne.produit.quantite + (vente.statut === 'complétée' ? ligne.quantite : 0);
    ...
    const produitsDisponibles = useMemo(() => {
        ...
        return produits.filter(p => !idsDejaSelectionnes.includes(p.id) || idsDejaSelectionnes.includes(p.id));
    }, [produits, lignes]);
                                                ...
                                                <SelectContent>
                                                    {produitsDisponibles.map((produit) => (
                                                        <SelectItem key={produit.id} value={produit.id?.toString() || `produit-${Math.random()}`}>
                                                            {produit.nom} - {formatMontant(produit.prix_vente)} FCFA (Stock: {produit.quantite})
                                                        </SelectItem>
                                                    ))}
                                                </SelectContent>
                                            </Select>
                                    ...
                                    {lignes.length === 0 ? (
                                        ...
                                        <div className="space-y-4">
                                            {lignes.map((ligne, index) => (
                                                <Card key={ligne.produit_id} className="border">
                                                    <CardContent className="p-4">
                                                        <div className="flex items-center justify-between">
                                                            <div className="flex-1">
                                                                <h4 className="font-semibold">{ligne.produit.nom}</h4>
                                                                <p className="text-sm text-muted-foreground">
                                                                    Stock disponible: {ligne.produit.quantite}
                                                            ...
                                                            <div className="flex items-center space-x-4">
                                                                ...
                                                                <div className="flex items-center space-x-2">
                                                                    ...
                                                                    <Input
                                                                        ...
                                                                        min="1"
                                                                        max={ligne.produit.quantite + (vente.statut === 'complétée' ? ligne.quantite : 0)}
                                                                    />
                                                                    <Button
                                                                        type="button"
                                                                        variant="outline"
